...
public class UserController {
	...
	public String handleLogin(User u, Model m, HttpServletRequest request) {
		User u1 = userService.loginUser(u.getEmail(), u.getPassword());
		if(u1.getId() != null) {
	...
	public String dashboard(Model m,HttpServletRequest request) {
		...
		List<Contact> contact = userService.getContact(userById);
	...
	public String logout(HttpServletRequest request, Model model) {
	    ...
	    model.addAttribute("login", new User()); // <-- add this line
	    return "loginPage";
	}
